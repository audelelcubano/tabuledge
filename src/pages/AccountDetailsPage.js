...
function Row({ label, value, mono = false }) {
  return (
    <div style={styles.row}>
      ...
      <div style={{ ...styles.value, fontFamily: mono ? "ui-monospace, SFMono-Regular, Menlo, monospace" : undefined }}>
        {value}
      </div>
    </div>
  );
          ...
          <>
            <div style={styles.header}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => navigate(`/ledger/${id}`)} title="Open ledger for this account">Go to Ledger</button>
            ...
            <section style={styles.card}>
              ...
              <Row label="Category" value={`${account.category} → ${account.subcategory}`} />
              ...
              <Row label="Status" value={account.active === false ? "Inactive" : "Active"} />
              <Row label="Created By" value={account.createdBy || "—"} />
              <Row label="Created At" value={account.createdAt?.toDate ? account.createdAt.toDate().toLocaleString() : "—"} />
              ...
              <div style={styles.moneyGrid}>
                <div>
                  ...
                  <div style={styles.moneyValue}>{formatMoney(account.initialBalance || 0)}</div>
                </div>
                <div>
                  <div style={styles.moneyLabel}>Debit</div>
                  <div style={styles.moneyValue}>{formatMoney(account.debit || 0)}</div>
                </div>
                <div>
                  <div style={styles.moneyLabel}>Credit</div>
                  <div style={styles.moneyValue}>{formatMoney(account.credit || 0)}</div>
                </div>
                <div>
                  <div style={styles.moneyLabel}>Balance</div>
                  <div style={{ ...styles.moneyValue, fontWeight: 700 }}>{formatMoney(account.balance || 0)}</div>
                </div>
              </div>
            </section>
          </>
...
const styles = {
  ...
  card: { background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 12, padding: 16 },
  row: { display: "grid", gridTemplateColumns: "180px 1fr", gap: 12, padding: "6px 0" },
  ...
  moneyLabel: { color: "#334155", fontSize: 12, textTransform: "uppercase", letterSpacing: 0.4 },
